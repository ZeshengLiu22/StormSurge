...
class AdditiveExceedanceHead(nn.Module):
    """Production direct-dual branches with y = body + excess.

    The window gate remains an auxiliary event predictor. Keep branch names,
    construction order and initialization identical to ExceedanceHead so that
    reconstruction is the only model-level variable in this study.
    """
    def __init__(self, hidden, dropout, threshold, prior, fixed_gate=False, head_hidden=None):
        super().__init__()
        threshold = torch.as_tensor(threshold, dtype=torch.float32).reshape(1, -1)
