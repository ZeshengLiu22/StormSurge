...
Every model prints [parameter counts](docs/MODEL_PARAMETERS.md) once at training/inference startup: `total`, `trainable`, `non_trainable`, and `head`. Counts are saved as `model_parameters` in checkpoints, training summaries, and inference metric reports, with backbone and head-branch details. DDP prints only on rank 0; counting requires no forward pass and excludes buffers.
...
DDP seeds each process with `seed + rank`; its samplers use the original default seed 0. Z-score statistics use the training device and distributed sums, preserving the original FP32/FP64 arithmetic order. Robust/mag statistics use TRAIN node samples; `x_nodes_per_graph <= 0` means at most 256 nodes per graph, without reducing the model's input graph. Augmentation at probability 1 skips the probability draw, preserving the original RNG sequence. The original lower bounds remain `1e-6` for `wmse_s`/`slope_mask_s` and `1e-12` for `slope_charb_eps`/`slope_huber_delta`; normal configured values are unaffected.

Single-process training uses a private DataLoader `torch.Generator`, seeded once from `--seed`. It advances across epochs independently of model initialization and dropout, so architectures with the same data and loader settings see identical minibatch orders. This intentionally changes historical single-process training trajectories. DDP sampler behavior and evaluation loader defaults remain unchanged. `build_loader(..., generator=None)` keeps the previous default for other callers.
