...
class ForcingTests(unittest.TestCase):
    def test_rectangular_grid_coordinates_and_pressure(self):
        ...
        original = raw.copy()
        actual, local_lat, local_lon = prepare_forcing(raw.reshape(-1, 3), lat, lon)
        ...
        np.testing.assert_array_equal(local_lat, lat[1:4])
        np.testing.assert_array_equal(local_lon, lon[1:4])
